...
import { RegistroMaterialiScadutiService } from "src/app/api/registro-materiali-scaduti.service";
...
@Component({
  selector: "app-registro-materiali-scaduti",
  templateUrl: "./registro-materiali-scaduti.component.html",
  styleUrls: ["./registro-materiali-scaduti.component.scss"],
})
export class RegistroMaterialiScadutiComponent
  ...
  // centriDiCosto: Observable<CentroDiCosto[]>;
  cdcs: CentroDiCosto[];
  ...
  constructor(
    ...
    private readonly client: RegistroMaterialiScadutiService,
  ...
  ngOnInit() {
    ...
    this.myCdc.valueChanges.subscribe((value) => {
      if (
        ...
        this.cdc.search(value).subscribe((res) => {
    ...
    this.myMateriale.valueChanges.subscribe((value) => {
      if (
        ...
        this.catService.search(value).subscribe((res) => {
  ...
  private _filterMat(value: string): NamedEntity[] {
    const filterValue = value.toLowerCase();

    return this.materiali.filter(
      (option) =>
        option.nome.toLowerCase().includes(filterValue) ||
        option.codice.toLowerCase().includes(filterValue)
    );
  }
  ...
  openDialog(row: MatScaduto, message: string, operation: string): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        // DO SOMETHING
        if (operation === "SAV") this.save(row);
      }
    });
  }
  ...
  save(row: MatScaduto) {
    ...
    return (
      this.client
        ...
        .toPromise()
        .then(() => {
          console.log(`MatScaduto aggiornato: ` + row.id);
        ...
        .catch((err) => {
          this.snackBar.open(
            "Errore nella modifica dei dati di ritiro",
            "Chiudi",
            { panelClass: "snack-error" }
          );
          this.saving = false;
